...
搜索调用与网页 HTTP 分开计数：搜索用于获取候选链接，网页请求用于访问来源；robots 检查、正文、重定向及实际发出的失败请求都消耗网页额度，同任务复用 robots 缓存不重复计数。因此 8 次网页请求不等于 8 次搜索，也不等于 8 篇文章。2026-09-14 的单次验证只用了 2/4 次搜索，但已用 8/8 次网页请求。9 月 15 日定点修复先在原 8 次上限内验证按组提前回退；12 次仅作为下一单公司受控验证建议和离线对照配置，尚未启用。若批准采用 12 次，搜索、字节、时限、模型、金额和访问规则仍保持原界限，不将增加尝试次数视为来源必然可读。
