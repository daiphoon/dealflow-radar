...
| `BochaSearchProvider` | R3 已实现，默认关闭 | R2 选定条件回退源；百度失败或准确主体结果不足时调用；已启用的增量按需路径还允许该组正文全部读取失败后沿用原查询回退，不做并行双查 |
...
E4.3 的 `same-query-readable-fallback-v1` 扩充上述回退触发条件：增量按需任务中，主源有相关候选但该组正文全失败时，可以在剩余预算内调用尚未使用的备用源。保持原两组查询，复用有效 robots 规则排除禁止路径；备用源也失败时保留缺口，不能把搜索摘录或初始导入资料冒充本轮已读取正文。

E4.4 新普通增量任务采用 `verified-short-business-topics-v2`：使用最短的已确认且无冲突业务别名，分别查询融资与产品；首次执行保存两组查询，恢复和回退沿用原文，旧任务与关注巡检保留旧策略。仅这两类记录定向搜索覆盖，其余类别显示未配置。博查按其[官方接口](https://open.bochaai.com/)的 `freshness=oneYear` 落实最近一年请求，不请求摘要；原本地近期性闸门继续执行。

E4.4 的 `same-query-readable-fallback-v2` 只用于上述短查询增量任务：每次读取前检查各组，某组候选已经全部尝试且全失败时，在剩余预算内提前使用该组备用源，不等待其他组读完。已处理的候选和证据顺序保持，备用结果先进入未处理部分；命中已经排队的 URL 时合并发现来源/查询组并前移，只读取一次。服务、支持、关于和联系等固定页面按 URL 路径后置，主源和备用源均先排序再限制候选名额；这只调整读取优先级，不改变来源等级、许可或事实质量判断。旧宽查询任务继续使用 v1，默认关闭路径及关注巡检不采用提前回退。

真实读取验证后的 `finance.eastmoney.com` 财经频道加入既有媒体原文排序，不扩大到同站自媒体。业务正文缺少机器可读发布日期时，仅接受与页标题匹配且连续的“标题—完整日期—来源”页头，保存原文本及日精度；不以正文事件日期或页脚日期代替。融资抽取优先保留完整连续正文，重复标题不构成多次融资，实质冲突仍保留候选；未披露轮次、发生日等字段保持未知。
