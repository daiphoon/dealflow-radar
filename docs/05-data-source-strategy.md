...
> 当前增量范围见 [ADR-0021](DECISIONS/ADR-0021-incremental-event-delivery.md) 和 [完整计划](15-incremental-event-delivery-plan.md)：先用已有材料完成一类业务事件，再补类别来源路由。下列协议/预留来源不等于全部已接通，九类检索标签不等于九类均有可靠数据。未配置、读取失败、受限、成功无记录与取得证据必须分别表达；本轮不新增 Provider 或采购。
