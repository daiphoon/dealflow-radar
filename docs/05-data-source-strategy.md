...
V1 安全抓取仍只解析 HTML、XHTML、RSS/Atom 和 XML，不解析 PDF。交易所 PDF 可以被识别为高优先正式披露地址，但在没有独立 PDF 安全解析和正文测试前不得据此生成事实；真实复测若证明 PDF 是主要漏报来源，再作为单独小里程碑处理，不能在本次来源排序修复中顺带扩大文件处理面。
