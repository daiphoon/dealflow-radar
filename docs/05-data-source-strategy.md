...
R3 使用 `SearchProvider` 发现候选 URL，再使用现有安全抓取边界读取原网页。R3.1 在百度适配器使用其正式支持的“最近一年”检索条件，并继续用本地可配置近期窗作最终判断；先按政府网站、已核验公司官网、经人工维护的小型媒体域名清单、其他可定位原文、资料/聚合页排序。该排序只决定有限抓取额度先看谁，不等于来源背书，也不能绕过正文证据闸门。明确过旧或未来日期的搜索结果降到队尾并触发条件回退；搜索结果日期不可靠或缺失时仍可抓取原页，由原页元数据作最终判断。
