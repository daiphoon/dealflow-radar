...
此入口独立于上方 JSON 路径，复用现有人工决定与证据表。前提为数据库已迁移至 `0032`、使用非 owner 应用连接，`IMPORT_USER_ID` 是 active 平台管理员，`IMPORT_TENANT_ID` 与该用户一致。不要求官网、政府正文或机构管理员身份；原已核验依据不覆盖。仅接收已获负责人确认的公开资料，机构内部投资、持仓和协议不属于此入口。
