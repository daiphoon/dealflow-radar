...
融资新材料保存为待核实观测：同事项补充来源、更正及字段差异在公司页可见，已确认初始资料继续保留。动态页面或验证码分别记为读取缺口，不自动增加调用或绕过限制。当前只通过 Mock/隔离 PostgreSQL 验收，生产部署、真实来源对照仍按 E4.3 预览执行；本节不是开启生产开关的授权。关闭新路径可回退处理逻辑，带融资观测时不得降级删除 `0033` 策略。
