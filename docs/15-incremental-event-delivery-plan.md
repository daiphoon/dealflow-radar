...
2026-09-13 已批准先合并 PR #80，再准备事件基准。当前交付是基准的选择、标注和执行条件；样本确定前不运行新的研究任务，不用工程样本替代真实价值验收。

**样本与冻结**：

- 保留原未上市公司的“来源不足、无已知事件”记录；它不是已经证实无事件的负样本，不进入召回率分母。原上市技术例外继续只作技术对照，不计未上市公司价值验证。
- 最小补充方案是 **1 家用户真实关注的未上市公司、1—3 条事前已知的重要事件**；优先采用已实现的中标类，合同相关材料必须区分中标、签约和履约。这是待确定的补充样本上限，不自动建公司或扩充真实测试。
...
| 指标 | 固定口径 |
| --- | --- |
| 召回 | 命中的独立基准事件数 / 范围内已标注事件数，同时报告 `n/N`；无真值或分母为 0 时为 `N/A`。小样本命中不能推广为一般未上市公司召回率 |
...
本次基准准备的产品搜索、网页研究、模型调用和新增现金预算均为 **0**。后续真实发现测试的建议边界为单家公司、一次业务研究，沿用现有上限 **4 次搜索、8 次网页请求、2,000,000 字节、180 秒，模型 Token 为 0**；是否能在 0 元现金额度内执行，须届时核对免费资源与价格证据。身份尚未核验时先完成独立的身份范围/预算确认，不能把历史特殊额度沿用到新公司，也不能把身份费用藏在业务研究上限之外。
