...
当前 `0015` Schema 已实现双通道共享查询、私有叠加、受控共享事实和 CloudBase 本地身份映射；基金不再是读取共享公司档案的前置。CloudBase 只证明用户身份，业务角色、基金权限和 RLS 继续由本地模型决定。个人关注、organization 完整模型、商业订阅和支付继续分阶段实施，详见[实施计划](10-implementation-plan.md)。
...
`company_aliases` 保存名称类型、有效期、来源、验证状态和独立作用域；`company_relationships` 保存母子公司、集团成员、品牌运营、产品归属等带方向和有效期的关系。全局目录公司经显式身份确认产生的已核验法定曾用名进入无 owner 的共享身份索引，当前全称、曾用名和信用代码都指向同一 `company_id`；租户私有公司、内部代号、品牌名和未核实名称不得因此共享。每次实体匹配保留候选集合、命中规则和置信度。官方记录冲突时不静默覆盖公司；人只选有关联的官方候选，事件重建与发布路由由程序完成。
