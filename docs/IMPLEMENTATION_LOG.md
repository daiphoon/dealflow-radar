...
- 实际命令：`uv run ruff check backend migrations scripts tests`；`uv run ruff format --check backend migrations scripts tests`；隔离 SQLite 空库 `alembic upgrade head`、`alembic check`和 `alembic downgrade base`；隔离 PostgreSQL 16 空库升级、`0015 → 0014 → 0015`往返、Schema 漂移、受限应用账户和 RLS 套件；`uv run pytest -q`；`npm run typecheck`、`npm run build`、`npm audit --audit-level=high`；本地 Mock CloudBase 的四角色登录、刷新、查询、私有叠加和退出浏览器冒烟；迁移前后 `pg_dump -Fc`、`pg_restore -l` 和 SHA-256 校验；`git diff --check`。
- 测试结果：173 项 Pytest 在 PostgreSQL RLS 环境全部通过，仅有 FastAPI TestClient 上游弃用警告；Ruff、格式、SQLite/PostgreSQL 迁移、Schema 漂移、TypeScript、Next.js 生产构建和依赖审计通过，0 个已知高风险漏洞。本地浏览器确认无基金个人、基金用户、其他 tenant 和平台管理员按本地授权得到不同结果，refresh token 轮换和退出成功，控制台无错误。本地试运行库从 `0012` 顺序升到 `0015`，升级前后均有可读备份，12 家公司、3 个基金、13 条投资、12 条事件、原文和证据数量不变；本轮真实 CloudBase、天眼查、付费 API 和模型调用均为 0，自动刷新与自动发布保持关闭。
- 未解决阻塞：当前没有可用的 `CLOUDBASE_ENV_ID`和四个真实受邀邮箱账户，因此真实邮件收码、CloudBase 会话过期和官方服务不可用场景尚未验收；不应把 Mock 浏览器结果宣称为真实 CloudBase 接入已上线。在这一真实验收完成前，M3 保持“进行中”，不开始 M4；FastAPI TestClient 上游弃用警告仍存在，不影响当前结果。
