...
- 未解决阻塞：生产仍在 `0021`，因为代码 PR 尚未合并。只读核查确认预计处置 12 个旧依据身份、21 条旧来源独占非撤回事件和 4 个关联已完成申请；4 份个人报告均不含旧供应商名称且未引用这些事件。生产 Secret、13 个私有缓存文件和旧部署环境变量必须等合并后完成升级前备份、`0022` 部署、真实账号/RLS 回归及升级后备份，再无输出地删除；必要审计和最近可回滚 release 不提前清理。搜索源准入与 R3 研究 Agent 继续后置。
