...
- 未解决阻塞/边界：M1 独立 PR 的远端 CI、最终固定 amd64 制品和生产集中授权仍待完成；未合并 M1 PR、未部署、未执行生产写入或真实研究/MCP。
...
- 实际命令：私有 `setup_isolated.py`、`remote_runner.py init/preview/create/run/capture`、`cancel_deferred.py`、`archive_isolated.py`；`ssh` 下只读 SQL 与生产最终核对，Chrome 只读查看博查/Tavily 额度，DeepSeek 余额端点只读查询，最后清理隔离容器/网络及临时密钥。两次隔离初始化参数/挂载错误均在外部调用前修正。
- 验证/结果：发现输出先封存再比照；同一事项线索 1/4，完整交付 0/4，11 条未确认候选、0 已发布，E4/EV14/M6B 不关闭。13 次搜索、18 次 HTTP、5 次模型，29 条用量账本均已结算，估算 ¥0.088046；免费额度与模型余额交叉核对。隔离库加密归档双哈希通过，未重复恢复演练；临时环境和密钥已清理。生产 17 项运行开关及两份环境各 23 项研究开关关闭、五项金额上限零、活动研究任务 0，原自动续作暂停；未运行新留出样本第二遍。
- 下一闸门：审查 E4.10 检索—证据—事项贯通修正的范围与验收；本轮只交付复测诊断，不启动新工程、PR、部署或下一次外部研究。

## 2026-09-23：PR #96 合并与可靠性改造受控部署

- 任务/关键文件：按负责人批准合并 [PR #96](https://github.com/daiphoon/dealflow-radar/pull/96)，将固定提交 `3e143d8` 部署到香港环境，数据库 `0034→0035`；同步 README、有效看板、交付记录及本记录，不增改产品代码或运行开关。
- 实际命令：`gh pr merge 96 --squash --match-head-commit`、`git fetch`、`git archive`；Tailscale SSH 执行私有 `prepare.sh/backup.sh/deploy.sh/post-backup.sh`，Mac 执行 `restore_local.py/verify_post_backup.py/final_checks.py`；Chrome 只读检查登录页。
- 验证：合并文件树等于已审查分支，最终 CI 后端 1039 通过/19 跳过、前端 28 通过。部署前 age 加密备份上传既有 COS，SSH 副本密文/解密流双哈希与隔离 PostgreSQL 16 恢复通过；`0034→0035` 演练旧行摘要一致、迁移无漂移、非 owner/RLS 通过。线上健康、就绪、登录页、匿名拒绝、旧行摘要和部署后加密备份双哈希通过；临时恢复库已删除。
- 边界：运行 17 项开关、两份环境各 23 项开关关闭，五项金额上限零，活动研究任务 0、原自动续作暂停；无本轮真实 Provider/模型调用或新样本复测。COS 直接下载及部署后隔离恢复本轮未重复；E4 原 3/9、1/4 未达标，E4/EV14/M6B 不关闭。私有回执在 `data/private/deployment/pr96-20260923/`。
...
- 实际命令：`gh pr edit --base main`、`gh pr merge --merge --match-head-commit`、`git merge-tree --write-tree`、`git archive`；SSH 执行私有 `prepare.sh`/`backup.sh`/`deploy.sh`，Mac `restore_local.py`；`run_import.py preview/apply/repeat/verify`；隔离库 `remote_runner.py preview/create/run/capture` 五家各一次及 `export_discovery.py`；`maintenance_replay.py`、`analyze_results.py`、Chrome 配额/页面核对。
- 验证：最终 CI 897 通过/18 跳过、前端 26 通过，部署树与验收树一致，`0033` 无漂移，部署前加密备份/COS 上传/SSH 副本双哈希和隔离恢复、切换摘要及权限/HTTP 检查通过。生产接收 24 家/139 条、65 确认/74 待核，原 10 条复用、3 条说明保留；重复导入新增事件 0，原事实/证据/观测/账本/任务保留，页面实见单家公司 8 条资料。
- 真实结果：独立发现仅含已确认身份和空业务库/缓存；固定分母 9、待核 1 排除，合格正文支持输出 3/9，开发样本 2/3、留出样本 1/6。6 条候选输出对应 5 个事项，IPO 辅导重复展示 1 条；额外进展不替代漏报，日期受阻的内部正文不计合格输出。17 搜索/40 HTTP/878,952 字节/零模型，配额百度 48→38、博查 965→958，免费额度内现金费用 0。
- 维护与限制：真实融资正文在另一隔离 PostgreSQL 受限角色回放，5 条初始事实/快照保持、自动补充 1 份材料、幂等和原件权限通过；首次测试脚本误将全部历史观测数当作新增数，修正为增量后在新隔离库复验，不改产品代码。生产导入临时目录权限/PYTHONPATH 预览错误已修复，均发生在写入前。1 家同信用代码的全称差异影响 6 条，单独等待确认。长期自动更新/巡检仍关闭，E4/EV14/M6B 未通过；下一建议及停止边界见看板，不追加研究或进入 E5。
- 收尾：`post-backup.sh`/`verify_post_backup.py` 完成操作后加密备份上传 COS、SSH 副本双哈希，未重复恢复或 COS 直接下载；`archive_isolated.py` 完成两个有效验证库加密归档与双哈希，未做归档恢复。随后停止/删除本轮临时数据库容器、清理三个临时环境文件，仅生产四个服务继续运行。真实资料及回执均留在私有忽略目录。

## 2026-09-20：E4.7 整体研究交付工程（本地，待合并部署与真实验收）

- 任务/关键文件：新增 `curated_batch.py`、`research_plan.py`、`company_refresh.py`；扩展原表八类接收与跨公司说明、实际选择摘要 v2、主题规划/正文调度/有界替代查询、过期访问入队、前端状态及禁止公司链接预加载。同步 ADR-0023、计划、看板、API/成本/运维说明；保留旧 Demo 路径，无新迁移或依赖。
...
- 任务/关键文件：按批准选择原 Excel 的第二家公司，冻结基准，调用现有人工确认入口导入 5 条资料，保留旧 2 条已发布资料；执行一次原生申请和 Worker。仅更新 README、看板、增量计划及本记录，无产品代码、迁移、依赖或长期配置变化。私有回执在 `data/private/e4-validation/20260920-e46-second-company/`。
- 实际命令：`CuratedWorkbookProvider` 生成单公司投影；`run_precheck.py`、`import_phase.py preview/apply`、`run_phase.py preview/create/business-run`、`capture_case.py sealed-discovery`、`read_after.py`、`snapshot_history.py before/after`；Chrome 核对免费额度及公司详情；`diagnose_offline.py` 调用现有 `_fetch_candidate` 复现文档名额停止；`verify_preservation.py`、运行/环境关闸检查、前后 `backup.sh`/`post-backup.sh` 与 `verify_backup.py` 双哈希验证。
- 结果：16:25 完成，2 百度搜索/0 博查/7 HTTP/507,944 字节/约 13.4 秒，模型及免费额度内现金费用零，配额 50→48/965→965。融资线索命中 1/1，融资正文支持 0/1、关联材料 0；IPO 取得正文生成 1 条平台未确认线索，页面显示 7 条事实和 1 条线索。取得三份文档（含两份内部底稿）触发文档上限，融资页仍在队列；离线零网络复现，未证明待读融资页可用或 E4.5 关联在新样本有效。
- 保护/收尾：42 张表旧行摘要与样本逐条核对通过，原 161 条账本不变，新增 7 条（1 条零调用导入、6 条研究结算）；仅目标身份更新、导入派生快照切换和正常查看时间改变旧行，原事实/证据/底稿保留。非 owner 权限、Chrome、输入/基准/封存哈希、前后加密备份上传及 SSH 副本双哈希通过；本轮不重复隔离恢复或 COS 直接下载。13 项运行和两份配置各 19 项研究相关开关关闭、无活动 Worker、自动续作暂停。
- 停止：本轮测试和诊断完成，EV14/M6B 仍未通过；下一建议在原预算下按主题优先读取事件文章并明确停止原因，待批准后实施。检索历史的旧组合标签和 IPO 与已有资料重复呈现另列缺口，不顺手扩展。未运行第二次研究或第三家公司，未进 E5。

## 2026-09-20：E4.5 合并部署及既有融资材料关联

- 任务/关键文件：按批准合并 PR #89/#90，将 `10baf8d` 部署至香港环境；固定两份既有材料关联并追加审计。同步 README、有效看板、增量计划及本记录，无新增产品代码、迁移或依赖。私有操作和回执在 `data/private/deployment/pr89-pr90-20260920/`。
- 实际命令：`gh run rerun` 重试 PR #89 的 Docker Hub 502；`git rebase --onto`、文件树比较、最终 CI 与精确提交合并；SSH/SCP 执行 `prepare.sh`、`deploy.sh`、`backup.sh`、`post-backup.sh`。Mac `restore_local.py` 完成双哈希、隔离恢复及历史关联演练；`remote_association.py preview/apply/verify` 执行一次正式关联；Chrome 展开两份对照材料；`final_checks.py`、`verify_post_backup.py` 与文档检查收尾。
- 结果：最终 CI 后端 868 通过/17 跳过、前端 25 通过；重排与合并文件树一致，15:31（北京时间）部署完成，`0033` 无漂移，切换前后全表摘要一致。线上 5 条初始事实保留、2 份材料关联、0 条重复线索、0 条新增事件；旧事件仅退出重复展示，原正文/证据/观测和目标事实不改。隔离演练覆盖幂等、坏预览拒绝、撤证及普通用户权限；线上受限角色与页面通过。
- 收尾/限制：本轮研究和模型调用为 0，申请、任务与账本保持；13 项运行开关、两份配置各 19 项研究相关开关关闭，5 项金额上限零、默认 HTTP 8/冷却 24 小时保持，自动续作仍暂停。部署前及关联后加密备份上传 COS、SSH 副本双哈希通过；部署前隔离恢复通过，操作后未重复恢复或 COS 直接下载。原输入/封存 7 份哈希保持。来源轮次与实际发生日未知，转载不视为独立确认；不改写历史召回，不启动第二家公司或 E5。

## 2026-09-15：E4.5 融资自动比对与补充来源

- 任务/关键文件：新增 `financing_comparison.py`，调整融资存储、详情投影/类型与页面；使用多字段与唯一事项判断处理缺轮次/相邻日报道，区分缺项、补充和差异，人工资料读取标题金额及明确投资方/发生日。新候选采用 `financing-v2`，旧版本可读可追加，事实、日期、快照、权限和发布资格保持。
- 实际命令：私有 `run_tests.py final` 运行最终规则、SQLite/PostgreSQL 非 owner 存储及真实封存回放；相关回归覆盖增量、回退、短查询、人工导入与详情。`pytest` 先复现同日不兼容材料的两个唯一键失败用例，再修复并验证；`ruff check/format --check`、`npm test`、`npm run typecheck`、`npm run build`、`validate_delivery.py` 检查公开差异、链接和封存哈希。
- 结果：最终定向 97 通过/2 条件跳过，相关 98 通过/2 条件跳过；前端 25 通过、类型与构建通过。真实回放五条原事实不变，两篇正文关联为两份未确认补充材料，新增事件 0，轮次仍未知。并发验收针对已初始化研究来源的事务写入。原材料/封存 7 份哈希保持，零公司网络/模型调用，无生产访问、迁移或新依赖。
- 限制/停止：只证明本地工程和封存回放，不计新召回；现存线上线索未回填，合并部署待确认。有 v2 数据后应用回退须保留其读取/撤证兼容。已知规则仅支持同日或相邻一天且有金额和共同投资方的缺项关联，依据不足仍保持候选，不扩样或启动 E5。

## 2026-09-15：E4.4 提前单次复测完成

- 任务/关键文件：按负责人明确授权以一次申请冷却特例替代晚间排程；仅私有运行器使用临时策略，同步 README、成本说明、有效看板、增量计划及本记录，无产品代码变更。
- 实际命令：私有 `run_phase.py preview/create/business-run` 各一次，`capture_case.py` 封存前后记录，`verify_close.py` 关闸核验；`read_after.py` 非 owner 读取，Chrome 核验两条平台线索及五条初始资料；`post-backup.sh` 加密备份上传 COS，`finalize_checks.py` 验证原记录与双哈希，文档链接/隐私及封存哈希校验。
- 结果：13:24 完成，1 搜索/10 HTTP/152,611 字节/15.108 秒/零模型/现金费用零；百度 50→50、博查 966→965。两篇正文及两条候选线索可读，金额与领投方匹配基准，轮次未知；可能同一公告的转载，不计为独立两笔事件。原目标 14 组旧记录、原 150 条账本及其他样本保持，新增 11 条账本。运行与持久化开关关闭、自动任务暂停，长期配置不变；备份双哈希通过，本次不重复恢复演练。
- 未解决：完整轮次匹配与 EV14/M6B 未通过；下一建议将新来源关联到已有融资事项，保留各自口径，等待批准，不重复搜索或扩样。
...
## 2026-09-24｜E4.10 端到端事项交付闭环本地改造

- 任务：对照本地 `6b2b802` 与四家 E4.9 `3e143d8`，保留 PR #96 有效修复和已有文档修改；先建立失败回归，完成附件范围本地实现。
- 关键文件：事项日期/验证/比较/人工基准适配、获取/抽取/调度/覆盖/存储/读取链、公司详情、正式回归、ADR-0026、交付记录 17。无新迁移，head `0035`；Node 24 独立本地提交 `778a15c`。
- 实际命令：关闭外部调用的正式 Pytest 定向及私有 `run-final.py` 三组完整收集运行、`rerun-group.py 1` 独立基库重跑；日期等价修正后全单元/事项链路复测；SQLite 升降级/漂移、PostgreSQL 准备及非 owner 测试；Node 容器 `npm ci/test/typecheck/build`、npm 11.9.0 audit、前后端/备份镜像构建、Compose/preflight、虚构身份的实际 Server Actions 验收；Ruff/格式、差异与私有边界检查。
- 测试结果：完整 1106 项中 1087 通过、19 条件跳过；最后日期等价相关复测 616 通过/1 条件跳过，不重复计数。前端 28 通过，审计 0 漏洞，生产式本地登录/公司页/退出及健康通过。10/100 事件的 SQLite 比较查询 46/226→29/29，详情查询 52/412→11/11。四家封存正文真实入库/API 规则回放为 2/3/3/0 条未确认事项，重复处理无新增；发生时间均未知，严格交付仍 0/4。日志/哈希在忽略目录 `data/private/e410-local/`。
- 未解决边界：完整封存搜索响应缺失，不能证明调度召回增益；新真实模型和独立业务验收未执行，复杂语义/弱锚点、已读版本后置，E4 不关闭。真实付费研究/生产写入/自动发布/推送合并部署均为 0；不自动进入下一阶段。

## 2026-09-24｜E4.10 独立 PR 前审查

- 任务/关键文件：负责人追加授权审查、提交、推送和创建独立 PR。审查本轮完整差异后，修正专业获取失败的可选日志访问、已读无事项的主题完成记录、任务时间导致缓存不复用、人工多日期区间解析异常；`@types/node` 从 26.1.1 固定到 24.13.6，同步锁文件和类型依赖。正式回归新增 7 个参数用例；细节见交付记录 17。
