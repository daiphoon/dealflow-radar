...
R3.3 的官方 PDF 读取仍受 HTTPS、允许域名、DNS/SSRF、重定向、robots、MIME 与文件签名、字节、页数和解析超时限制；不执行脚本、附件或嵌入内容。R3.4 才在现有 PostgreSQL 血缘模型上补充原子事实及逐条支持状态，不新建 JSONL 状态库。R3.5 才把固定研究计划升级为最多一次缺口补查的受限循环；只有存在新增合格证据时才允许模型按版本化 Schema 做结构化解读，不生成用户未请求的完整报告。
