...
历史授权商业工商身份 V1 复用同一导入、候选和重路由服务，并以 `verification_basis=licensed_business_data` 与政府来源分开审计。该天眼查适配器现已被 ADR-0018 列为待退役旧实现，专用开关必须保持关闭，新功能不得调用或扩展它。历史证据不能改称政府或公开网络证据；活动代码、Secret、缓存和专用数据库行为由后续独立里程碑安全退役。

### 4.1 目标受限公开网络研究（已决策，尚未实现）

ADR-0018 确认后，后续新公司研究继续复用 PostgreSQL 任务、租约、取消、断线恢复、可信抓取、证据血缘、RLS 和用量台账，不引入第二套 Agent 状态存储。目标 Worker 依序执行：身份闸门 → 缓存/预算 → 固定投资研究计划 → 受限搜索 → 安全抓取 → 确定性解析/去重 → 必要时模型结构化抽取 → 证据和作用域校验 → 分级事实或待核实线索。

单次运行必须有搜索次数、补充轮数、URL、文档、下载字节、超时、Token 和费用上限。Agent 只可调用白名单搜索、受控抓取、读取共享事实和提交结构化候选等工具；不得执行 Shell/任意 SQL、确认公司身份、读取私有投资数据、修改权限或直接发布事实。同步公司页面继续只读数据库。详见 [ADR-0018](DECISIONS/ADR-0018-provider-neutral-bounded-web-research.md)。
