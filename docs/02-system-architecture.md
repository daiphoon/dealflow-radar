...
> 当前演进决策为 [ADR-0021](DECISIONS/ADR-0021-incremental-event-delivery.md)，完整范围与代码基线见 [增量交付计划](15-incremental-event-delivery-plan.md)。下方完整调度、差分和多来源归并流程包含目标设计，不能整体视为已实现；执行状态只看 [实施看板](10-implementation-plan.md)。
