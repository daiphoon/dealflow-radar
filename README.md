...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已合并并部署到香港环境：活动适配器、Worker、CLI、调用开关、生产 Secret、缓存和私有一次性脚本均已移除；向前迁移已保守停止展示仅由旧来源支撑的身份和事实，同时保留必要审计血缘。当前进入 R2 搜索源准入实测；供应商中立、预算受限、证据优先的公开网络研究尚未编码。
...
本机和香港环境均已运行 PostgreSQL 16 当前迁移，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 数据源架构转换的 R0、R1 已完成，当前进入 R2 搜索源准入，再实施 R3 并恢复独立用户的新公司研究复测。现有来源监测仍不是通用全网搜索，也不会自动生成或共享事实。
...
M6A-3 的供应商专用按需研究 Worker 已退出活动代码。同步公司搜索和详情始终只读 PostgreSQL；共享目录没有结果时，用户仍须提交准确工商全称或统一社会信用代码。申请会被持久化并可取消、可在重新登录后继续查看，但在新的搜索源完成准入且 R3 Worker 交付前会保持“等待新研究能力”的状态，不会自动创建公司或调用外部服务。未来排队、断线恢复、取消、共享缓存、证据分层和“未申请不生成报告”等通用规则继续保留。
