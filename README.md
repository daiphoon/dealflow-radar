...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3 由 PR #46—#49 完成并已部署到香港生产 `0023`。R3.1 的 PR #51—#53 已合并部署：正文噪声、过旧资料页和不安全来源被阻断，百度主源也已在生产真实调用中验证可用。R3.2 由 PR #54、#56 完成，已在不改变搜索调用和缓存指纹的前提下修复全角/半角主体匹配及上市阶段高价值变化召回。受控缓存回放进一步证明，部分高价值搜索结果落在微信、百家号等禁止普通自动客户端读取正文的页面；搜索 API 返回网址不等于目标网站授权抓取正文。当前唯一工程里程碑是 R3.3：遵守 robots，不绕过限制，建立原始来源回溯、合规证据通路和受限官方 PDF 读取；本阶段不引入大模型 Agent、新 Provider 或新公司样本。
...
- 已核验且允许进入共享目录的公司，可由平台管理员将个人申请接入受限公开网络研究队列；Worker 固定执行九类投资研究覆盖，优先复用 14 天共享搜索缓存，百度为主源、博查只按条件回退，搜索摘要只作发现线索，必须安全抓取原网页后才能形成带证据的分级候选；
...
本机和香港环境均使用 PostgreSQL 16，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 的 R0—R3.2 工程链路已完成，但 M6B 仍是 `修复后复测`：搜索召回已不再误拒绝目标主体，高价值内容能否形成可核验证据仍受 robots、原始来源定位和官方 PDF 读取能力限制。既有可信来源监测与 R3 受限公开研究是两条独立后台流程，都不会在同步页面联网或自动发布已核实事实。
