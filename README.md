...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3—R3.3 已完成并部署。R3.4 由 PR #60 完成，香港生产已升级到 `0024`；原子事实与证据逐条支持账本、保守历史回填、跨作用域隔离、零调用缓存回放和真实页面验收均已通过。当前唯一工程里程碑是 R3.5：在既有 PostgreSQL 任务、预算、取消和恢复边界内，最多执行一次关键证据缺口补查；只有新增合格证据存在时才允许模型按严格结构输出候选解读，不自动发布或生成用户未请求的长报告。
