...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3 由 PR #46—#49 完成并已部署到香港生产 `0023`。R3.1 的 PR #51、#52 已合并部署，北京智齿博创隔离缓存回放确认旧噪声不再形成候选。卧安机器人首次新公司实测通过身份、队列、权限和零模型调用安全验收，但百度主源因认证失败未生效，博查只返回一条明确过旧的爱企查资料页，内容价值验收未通过。当前只修复主源配置、抓取前排除旧资料页并加强官网/监管披露发现，再复测同一家公司；通过前不扩大邀请测试。
...
本机和香港环境均使用 PostgreSQL 16，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 的 R0—R3 工程链路已完成，但 M6B 仍是 `修复后复测`：R3.1 的代码闸门已经阻断正文噪声，生产实测还需证明主源可用并能找到至少一条主体、时间、变化和证据同时成立的真实候选。既有可信来源监测与 R3 受限公开研究是两条独立后台流程，都不会在同步页面联网或自动发布已核实事实。
