...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3—R3.2 已完成并部署。R3.3 由 PR #58 完成：搜索发现、目标页访问和已取得证据分层记录，robots 阻断摘要不生成事实，官方 PDF 受限解析。香港生产缓存回放中搜索调用、网页下载、模型 Token、新事件和自动发布均为 0，证据缺口可审计。当前唯一工程里程碑是 R3.4：在现有 PostgreSQL 血缘上建立原子事实与证据的逐条支持账本；本阶段不新增搜索、大模型 Agent、自动发布或长报告。
