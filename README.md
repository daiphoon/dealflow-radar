...
2026-09-23：[PR #96](https://github.com/daiphoon/dealflow-radar/pull/96) 已合并并受控部署，线上固定提交 `3e143d8`、数据库 `0035`。最终 CI 后端 1039 通过/19 条件跳过、前端 28 通过；部署前加密备份与隔离恢复、旧记录保护、受限账户/RLS、部署后健康与备份校验均通过。研究、模型、自动刷新及自动发布开关保持关闭。本批修正资料到事项的证据、归并与当前展示可靠性，**未执行新的真实召回复测**；E4.7 的 3/9、E4.8 的 1/4 仍为历史价值基线，E4/EV14/M6B 未通过。详见[交付记录](docs/16-matter-reliability-delivery.md)及[有效看板](docs/10-implementation-plan.md#2-有效执行检查点)。

当前为 `DEMO / VALIDATION`，第 2 阶段最小数据闭环已经验收，并已补充个人安全查询、按需缓存、Mock Worker V1、人工研究导入 V1 与受控可信来源监测 V1：

> **当前推进方向**：[ADR-0021](docs/DECISIONS/ADR-0021-incremental-event-delivery.md) 保留“增量交付＋局部重构”，[ADR-0022](docs/DECISIONS/ADR-0022-curated-baseline-and-incremental-research.md) 采用“负责人复核初始资料＋按需增量研究”。E4.7 工程已随 PR #91/#92/#93 合并部署为 `de50fe4`；生产接收 24 家、139 条资料，另 1 家的 6 条因现有全称差异单独待确认，无每公司五条限制。固定 5 家独立发现使用 17 搜索/40 HTTP/零模型，合格正文支持输出 **3/9**，未达 80% 目标；零联网维护回放验证实际新正文能补充已有事项、旧事实保持。长期自动更新仍关闭，E4/EV14/M6B 尚未通过。[完整计划](docs/15-incremental-event-delivery-plan.md)固定范围，[实施看板](docs/10-implementation-plan.md)维护唯一推进点。
...
- 已核验且允许进入共享目录的公司，可由平台管理员将个人申请接入受限公开网络研究队列；Worker 按实际配置记录类别覆盖（启用新主题策略时从八类中每次选择两类并按历史轮换，首轮为融资与退出；未执行类别不声称已检查），优先复用 14 天共享搜索缓存，百度为主源、博查只按条件回退，搜索摘要只作发现线索，必须安全抓取原网页后才能形成带证据的分级候选；
...
访问 `/watchlist` 查看当前账户的关注、研究阶段和申请。关注只允许已核验平台共享公司，不授予公司或基金数据访问权；搜索无结果时提交准确工商全称或信用代码，只会同步写入 PostgreSQL，不会在网页请求中调用搜索、模型或发布事实。R3 Worker 需要独立的四重调用开关和 Provider 密钥才可运行，默认全部关闭；个人关注和用量仅本人可见，平台管理员处理已授权的用户申请；启用 E4.7 后，过期访问可代用户自动申请后台更新。
