...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已合并并部署到香港环境；R2 已完成准入决策，采用“百度主源、博查仅在主源失败或主体结果不足时回退”。R3 受限公开网络研究 V1 已在当前代码 PR 中实现，等待评审、合并和受控生产验收；尚未对真实 Provider 发起调用。
...
本机和香港环境均使用 PostgreSQL 16，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 数据源架构转换的 R0—R2 已完成，R3 当前等待代码 PR 验收，合并部署后再恢复独立用户的新公司研究复测。既有可信来源监测与 R3 受限公开研究是两条独立后台流程，都不会在同步页面联网或自动发布事实。
