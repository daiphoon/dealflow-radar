...
- M5B 前置配置支持不暴露数据库端口的上海单机拓扑，并强制先生成客户端加密备份，再交给预配置的 COSCLI 上传；尚未创建任何云资源；
- 外部搜索、模型、付费 API、自动刷新、自动发布和审核工作台默认关闭。

本机已完成 PostgreSQL 16 迁移、Schema 漂移检查、非表所有者 `NOBYPASSRLS` 账户的租户/基金/个人隔离，以及个人变化、报告和基金私有叠加的浏览器闭环；真实 CloudBase 无基金个人已完成收码、登录、刷新、退出和共享查询验收。M5A 的本机生产式验收不等于真实部署：上海服务器、域名与备案、真实 HTTPS、异机加密备份、告警和另外三类 CloudBase 账户仍须在 M5B 验收。因此当前版本不能公开上线或邀请外部用户；来源监测也没有通用全网搜索、语义事实自动生成或自动共享。
...
已确认邀请测试采用一台上海轻量服务器承载应用和 PostgreSQL，数据库仅加入内部容器网络；备份必须先用 `age` 公钥加密，COS 上传脚本拒绝明文 `.dump`。以下命令只检查配置结构，不会购买资源、连接 COS 或启动服务：
...
示例文件包含故意保留的占位值，不能直接用于生产。真实部署、密钥生成、COSCLI 最小权限配置和恢复演练见[运维手册](docs/12-operations-runbook.md#m5b-上海环境部署顺序尚未授权执行)，部署取舍见 [ADR-0013](docs/DECISIONS/ADR-0013-single-host-invitation-deployment.md)。候选主域名是 `dealflowradar.cn`，在完成购买和备案前不得对外宣称已启用。
