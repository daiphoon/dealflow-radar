...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3 由 PR #46—#49 完成并已部署到香港生产 `0023`。北京智齿博创真实任务证明队列、进度、断线恢复、主备回退和安全抓取可运行，但也暴露了正文噪声、静态资料页优先和关键词误分类。当前唯一工程里程碑是 R3.1 “证据质量与近期重要变化闸门”；通过前不扩大邀请测试或查询新公司。
...
本机和香港环境均使用 PostgreSQL 16，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 的 R0—R3 工程链路已完成，但 M6B 仍是 `修复后复测`：R3.1 必须先确保平台内部发现文档只在正文、主体、时间和变化语义同时达标后才进入用户可见待核实层。既有可信来源监测与 R3 受限公开研究是两条独立后台流程，都不会在同步页面联网或自动发布已核实事实。
