...
- 外部搜索、模型、付费 API、自动刷新和审核工作台默认关闭；自动发布策略默认启用，但 URL 外部检查关闭时会安全降级为未确认线索。
...
访问 `http://127.0.0.1:3000`。前端默认使用虚构机构管理员身份；API 调试可访问 `http://127.0.0.1:8000/docs`。公司详情分别展示已发布事实和未确认线索，并显示自动发布路由、证据链接健康状态和日期精度。
