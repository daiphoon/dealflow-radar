...
> **当前架构转换**：ADR-0018 已决定停止以天眼查作为后续研究数据源。现存集成保持关闭，等待独立安全退役；新研究路线改为供应商中立、预算受限、证据优先的公开网络后台研究。该目标尚未编码，搜索 Provider 也尚未通过准入验证。
...
- 政府官方与历史授权商业工商身份使用不同核验依据；天眼查 V1 属于待退役旧实现，不得再启用或扩展；
...
本机和香港环境均已运行 PostgreSQL 16 当前迁移，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 当前先完成数据源架构转换，再恢复独立用户的新公司研究复测。现有来源监测仍不是通用全网搜索，也不会自动生成或共享事实。
...
M6A-3 的旧按需研究实现保持关闭并等待安全退役。同步公司搜索和详情始终只读 PostgreSQL；共享目录没有结果时，用户仍须提交准确工商全称或统一社会信用代码。ADR-0018 已确认后续采用供应商中立的后台受限研究，但该目标尚未编码，当前不会自动处理新申请。未来排队、断线恢复、取消、共享缓存、证据分层和“未申请不生成报告”等通用规则继续保留；天眼查模块、专用预算和缓存不再启用或扩展。

访问 `/watchlist` 查看当前账户的关注和申请。关注只允许已核验平台共享公司，不授予公司或基金数据访问权；搜索无结果时提交准确工商全称或信用代码，只会同步写入 PostgreSQL，不会在网页请求中调用搜索、模型或发布事实。当前自动研究保持关闭；新的受限研究 Worker 必须通过 ADR-0018 后续闸门后才能启用。个人关注和用量仅本人可见；用户明确提交的申请允许平台管理员处理。
...
历史天眼查授权工商身份 V1 使用 `licensed_business_data` 与政府来源分开审计。ADR-0018 已取代其活动集成方向：专用开关必须保持关闭，不得再运行旧查询命令；旧记录不能改称政府或公开网络证据。代码、Secret、缓存、文案和专用数据库行为将在独立退役里程碑处理，已应用迁移和必要审计历史不回写。
