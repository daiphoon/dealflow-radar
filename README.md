...
> **当前推进方向**：[ADR-0021](docs/DECISIONS/ADR-0021-incremental-event-delivery.md) 已确认“增量交付＋局部重构”。保留现有系统，先补单类业务事件闭环，再补来源覆盖/成本和关注公司的低频检查，随后恢复真实价值复测；机构能力条件后置。[完整计划](docs/15-incremental-event-delivery-plan.md)固定范围与验收，[实施看板](docs/10-implementation-plan.md)是当前状态和下一唯一切片入口。局部离线验证不代表完整功能已实现或生产已更新；旧供应商退役、身份、权限和发布边界继续有效。
...
迁移 `0028/0029` 仅在隔离测试库执行，尚未提交或部署；关闭开关可停止新写入，保留已有历史，有观测/审核记录时禁止丢弃历史的降级。此入口尚未接到通用公开网络研究 Worker，未验证真实来源覆盖，不支持任意网页或多标段表格。完整边界与下一切片见[实施看板](docs/10-implementation-plan.md)。
...
业务研究和主体查证已接入费用状态、调用前预占、结算及中断保护；未知价格保持未知，新金额上限默认零，真实调用仍需原开关和具体预算授权。平台管理员可使用默认只读的费用核对命令，详见[成本说明](docs/06-cost-control.md)。迁移 `0030` 仅在隔离测试库验证，尚未部署；模型解读、机构来源和基础设施不在此次原子预算范围内。阶段状态与停止点仍以[实施看板](docs/10-implementation-plan.md)为准。
