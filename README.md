...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已从活动代码和部署配置中移除专用适配器、Worker、CLI、缓存挂载及调用开关，并通过向前迁移保守停止展示仅由该来源支撑的身份和事实；生产 Secret 与缓存只在 R1 合并部署并完成备份后删除。新研究路线改为供应商中立、预算受限、证据优先的公开网络后台研究，该目标尚未编码，搜索 Provider 也尚未通过准入验证。
