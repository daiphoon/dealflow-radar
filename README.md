...
本机已完成 PostgreSQL 16 迁移、Schema 漂移检查、非表所有者 `NOBYPASSRLS` 账户的租户/基金/个人隔离，以及个人变化、报告和基金私有叠加的浏览器闭环；真实 CloudBase 无基金个人已完成收码、登录、刷新、退出和共享查询验收。该版本仍不能视为生产可用：其他三类真实 CloudBase 角色回归、生产部署、邀请制真实用户价值验证与商业订阅权益尚未完成；来源监测也没有通用全网搜索、语义事实自动生成或自动共享。
