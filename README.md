...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1 已安全退役旧集成，R2 确定“百度主源、博查条件回退”，R3 由 PR #46—#49 完成并已部署到香港生产 `0023`。R3.1 的 PR #51—#53 已合并部署：正文噪声、过旧资料页和不安全来源被阻断，百度主源也已在生产真实调用中验证可用。卧安机器人复测发现，两个搜索源其实已返回上市递表、通过聆讯、备案和招股等高价值变化，但程序因全角/半角括号匹配差异和上市阶段词不完整而错误过滤。当前唯一工程里程碑是 R3.2：只修复主体匹配与高价值变化召回，再使用卧安机器人现有缓存做零搜索回放；通过前不新增 Provider、模型或公司样本。
...
本机和香港环境均使用 PostgreSQL 16，非表所有者 `NOBYPASSRLS` 账户继续执行租户、基金和个人隔离。香港环境已经完成真实 HTTPS、端口收口、主机重启恢复、COS 加密备份恢复和四类 CloudBase 账户浏览器回归；腾讯云系统盘告警、三运营商免费拨测和飞书运维通知已经建立。M5B 已关闭；M6 的 R0—R3 工程链路已完成，但 M6B 仍是 `修复后复测`：R3.1 已证明安全拒绝噪声和过旧页面，R3.2 需要在不放松证据闸门、不增加搜索调用的前提下，召回搜索结果中已存在的真实高价值变化。既有可信来源监测与 R3 受限公开研究是两条独立后台流程，都不会在同步页面联网或自动发布已核实事实。
