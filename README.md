...
> **当前架构转换**：ADR-0018 已决定停止旧商业数据供应商路线。R1—R3.5 均已完成并部署；香港生产数据库已升级到 `0025`。系统在既有 PostgreSQL 任务、预算、取消和恢复边界内，最多执行一次关键证据缺口补查；只有本次新增且完成主体绑定的合格证据才允许模型按严格结构输出待核实候选解读，不自动发布或生成用户未请求的长报告。部署前加密备份、数据库迁移、卧安机器人隔离缓存零调用回放和真实账号页面验收均已通过；下一闸门回到 M6B 独立邀请用户复测，不继续扩张研究 Agent。
