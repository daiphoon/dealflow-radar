...
M6A-3 的按需研究基础默认关闭。同步公司搜索和详情始终只读 PostgreSQL；共享目录没有结果时，用户提交准确工商全称或统一社会信用代码，后台先核验主体并等待用户确认，再创建或复用全局公司研究任务。排队和状态不会因退出登录中断；取消前没有外部调用时只退还月额度，日提交次数不退。Worker 先查完整缓存再消耗外部预算，个人页不暴露精确缓存活动或其他客户私有档案的存在。PR 1 只交付身份确认、额度、队列和取消恢复；六大研究模块与分级事实展示属于后续 PR 2。在 PR 2 合并并完成受控部署前，不得把 `ON_DEMAND_RESEARCH_ENABLED`、`EXTERNAL_CALLS_ENABLED` 或 `TIANYANCHA_IDENTITY_CALLS_ENABLED` 打开。

访问 `/watchlist` 查看当前账户的关注和申请。关注只允许已核验平台共享公司，不授予公司或基金数据访问权；搜索无结果时提交准确工商全称或信用代码，只会同步写入 PostgreSQL 队列，不会在网页请求中调用天眼查、模型或发布事实。按需功能默认关闭；开启后由独立 Worker 先核验身份并等待用户确认，再创建或复用全局研究任务。个人关注和用量仅本人可见；用户明确提交的申请允许平台管理员处理。
