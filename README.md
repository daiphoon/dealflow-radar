...
- 登录测试用户可按信用代码、工商全称或已核实别名精确查询平台共享公司，无需基金关系；
...
- M5B 前置配置支持不暴露数据库端口的单机拓扑，并强制先生成客户端加密备份，再交给预配置的 COSCLI 上传；ADR-0014 已确定先部署腾讯云中国香港邀请测试环境，尚未创建任何云资源；
- 外部搜索、模型、付费 API、自动刷新、自动发布和审核工作台默认关闭。

本机已完成 PostgreSQL 16 迁移、Schema 漂移检查、非表所有者 `NOBYPASSRLS` 账户的租户/基金/个人隔离，以及个人变化、报告和基金私有叠加的浏览器闭环；真实 CloudBase 无基金个人已完成收码、登录、刷新、退出和共享查询验收。M5A 的本机生产式验收不等于真实部署：中国香港云服务器、真实 HTTPS、异机加密备份、告警、大陆网络质量和另外三类 CloudBase 账户仍须在 M5B 验收。因此当前版本不能开始外部邀请测试；来源监测也没有通用全网搜索、语义事实自动生成或自动共享。
...
已确认邀请测试先采用一台腾讯云中国香港服务器承载应用和 PostgreSQL，数据库仅加入内部容器网络；备份必须先用 `age` 公钥加密，COS 上传脚本拒绝明文 `.dump`。以下命令只检查配置结构，不会购买资源、连接 COS 或启动服务：
...
示例文件包含故意保留的占位值，不能直接用于生产。真实部署、密钥生成、COSCLI 最小权限配置和恢复演练见[运维手册](docs/12-operations-runbook.md#m5b-香港环境部署顺序尚未购买资源)，部署取舍见 [ADR-0014](docs/DECISIONS/ADR-0014-hong-kong-invitation-deployment.md)。应用入口计划使用 `app.dealflowradar.cn`；中国香港服务器不以大陆 ICP 备案为前置，但正式迁入大陆仍须企业备案。
