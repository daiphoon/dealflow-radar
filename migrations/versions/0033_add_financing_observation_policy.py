"""Allow bounded public research observations without exposing restricted raw documents."""
...
def upgrade():
    ...
    allowed = f"""
        event_observations.schema_version='financing-disclosure-v1'
        ...
        AND EXISTS (
          ...
          WHERE e.id=event_observations.event_id
            AND e.visibility_scope='platform_shared' AND e.event_type='financing_cap_table'
    ...
    op.execute(
        "CREATE POLICY event_observations_financing_read ON event_observations "
        f"FOR SELECT USING ({allowed})"
    )
    op.execute(
        "CREATE POLICY event_observations_financing_insert ON event_observations "
...
def downgrade():
    if op.get_context().as_sql or op.get_bind().scalar(
        sa.text(
            "SELECT count(*) FROM event_observations WHERE schema_version='financing-disclosure-v1'"
        )
    ):
        raise RuntimeError("Retain financing observations and their access policy")
    if op.get_bind().dialect.name == "postgresql":
        op.execute("DROP POLICY event_observations_financing_insert ON event_observations")
        op.execute("DROP POLICY event_observations_financing_read ON event_observations")
